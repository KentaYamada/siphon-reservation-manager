...
export class NewYearDishesSettingService {
  ...
  static save(payload: NewYearDishesSetting): Observable<void> {
    ...
    const data: firebase.firestore.DocumentData = {
      ...
      delivery_time_from: payload.delivery_time_from,
      delivery_time_to: payload.delivery_time_to,
    ...
    return from(docRef.get()).pipe(
      ...
      map(snapshot => {
        return {
          ...
          delivery_date: snapshot.data()?.delivery_date.toDate(),
          delivery_time_from: snapshot.data()?.delivery_time_from.toDate(),
          delivery_time_to: snapshot.data()?.delivery_time_to.toDate(),
